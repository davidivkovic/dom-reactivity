...
import { ExtendedElements } from "../dom/constants";
...
export function registerImportMethod(path, name, moduleName) {
  ...
  const importDeclarations = program.node.body.filter(
    node => t.isImportDeclaration(node) && node.source.value === moduleName
  );
  ...
  if (!importDeclarations.length) {
    return addNamed(path, name, moduleName, { nameHint: "_$" + name });
  }
  ...
  if (!specifier) {
    specifier = t.importSpecifier(t.identifier("_$" + name), t.identifier(name))
...
export function isExtendedElement(node) {
  ...
  if (isString) return ExtendedElements.includes(node)
