...
function registerImportMethod(path, name, moduleName) {
  ...
  // find import declarations of the program with the same moduleName (e.g. "dom-reactivity")
  const importDeclarations = program.node.body.filter(
    node => t__namespace.isImportDeclaration(node) && node.source.value === moduleName
  );

  // if there is no import declaration with the same moduleName, add one 
  // and return the import specifier local identifier (e.g. _$createComponent)
  if (!importDeclarations.length) {
    return helperModuleImports.addNamed(path, name, moduleName, { nameHint: "_$" + name });
  }
  
  // find the import specifier with the same name (e.g. "createComponent")
  let specifier = importDeclarations[0].specifiers.find(specifier => specifier.imported.name === name);

  // if there is no import specifier with the same name, add one (e.g. import { createComponent as _$createComponent } from "dom-reactivity")
  if (!specifier) {
    specifier = t__namespace.importSpecifier(t__namespace.identifier("_$" + name), t__namespace.identifier(name));
    importDeclarations[0].specifiers.push(specifier);
  }

  // return the import specifier local identifier (e.g. _$createComponent)
...
function isExtendedElement(node) {
  return node.openingElement && ExtendedElements.includes(node.openingElement.name.name)
}
...
function transformNode(path, info = {}) {
  ...
  if (isExtendedElement(node)) {
    return transformExtendedElement(config, path, info);
  }
  else if (t__namespace.isJSXElement(node)) {
...
function transformExtendedElement(config, path, info = {}) {
  /* 
    Transform 
    <if>...</if> into <If>...</If> 
    <for>...</for> into <For>...</For>
  */
  const node = path.node;
  node.openingElement.name.name = capitalize(node.openingElement.name.name);
  if (node.closingElement.name) {
    node.closingElement.name.name = capitalize(node.closingElement.name.name);
  }
  return transformElement(config, path, info);
}
